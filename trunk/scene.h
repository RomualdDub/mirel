...
{
    public:
    Scene(Gfx* ingfx, int inherox, int inpetx, int inlevel, Game* ingame, Map* inmap);
    ~Scene();
...
Scene::Scene(Gfx* ingfx, int inherox, int inpetx, int inlevel, Game* ingame, Map* inmap)
{
    gfx = ingfx;
    game = ingame;
    map = inmap;
    ...
    hero = new Hero(gfx, inherox);
    map->addhero(hero);

    pet = new Pet(gfx, inpetx, inlevel);
    map->addpet(pet);
    ...
    {
        mobs.push_back(new Mob(gfx, 1, 100, 1000, hero));
        map->addmob(mobs[i]);
    }
    mobrepop = 0;
    ...
    boss.push_back(new Boss(gfx, 5, 800, 1000, hero));
    map->addboss(boss[0]);
    bossrepop = 0;
}
...
void Scene::animate(int distance, sf::View* view)
{
                ...
                if (magics[j]->type == gfx->magic_spell0)
                {
                    if (mobs[i]->collide(magics[j]))
                    {
                        ...
                        if (mobs[i]->hp <= 0) // mob dead
                        {
                            ...
                            for (int k = 0; k < 14; k++)
                                particles.push_back(new Particle(gfx, mobs[i]->x, mobs[i]->y));
            ...
            if (pet->collide(mobs[i]) && !mobs[i]->hit && pet->hp)
            {
                ...
                if (mobs[i]->aggro)
                {
                    ...
                    if (pet->visible)
                    {
                        ...
                        for (int j = 0; j < 7; j++)
                            particles.push_back(new Particle(gfx, pet->x, pet->y));
                    }
                }
                ...
                if (magics[i]->type == gfx->magic_spell0)
                {
                    if (boss[0]->collide(magics[i]))
                    {
                        ...
                        if (boss[0]->hp <= 0) // boss dead
                        {
                            ...
                            for (int j = 0; j < 14; j++)
                                particles.push_back(new Particle(gfx, boss[0]->x, boss[0]->y));
        ...
        for (int i = 0; i < (int)mobs.size(); i++)
        {
            if (mobs[i]->visible)
            {
                if (mobs[i]->collide() && mobs[i]->aggro)
                {
                    ...
                    for (int j = 0; j < 7; j++)
                        particles.push_back(new Particle(gfx, hero->x, hero->y));
                }
            }
        }
    }
    ...
    if ((int)mobs.size() != MAXMOBS)
    {
        ...
        {
            mobrepop = 0;
            Mob* mob = new Mob(gfx, 1, 100, 1000, hero);
            mobs.push_back(mob);
            map->addmob(mob);
        }
    }
