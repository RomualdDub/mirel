...
{
    public:
    Scene(int inherox, int inpetx, Map* map);
    ~Scene();
...
Scene::Scene(int inherox, int inpetx, Map* map)
{
    world = new World;
...
void Scene::animate(int distance, sf::View* view, Game* game, Map* map)
{
    ...
    if (pet->y < ground[(int)pet->x] - 64 + 8)
        pet->y += distance;
    if (pet->y > ground[(int)pet->x] - 64 + 8)
        pet->y -= distance;
    ...
    for (int i = 0; i < (int)mobs.size(); i++)
    {
        if (mobs[i]->y < ground[(int)(mobs[i]->x)] - 64 + 8)
            mobs[i]->y += distance;
        if (mobs[i]->y > ground[(int)(mobs[i]->x)] - 64 + 8)
            mobs[i]->y -= distance;
    }
...
void Scene::keyaction(Game* game)
{
    if (!hero->attack)
    {
        if (game->mana >= game->level * 5)
        {
            ...
            if (game->mana > 0)
            {
                magics.push_back(new Magic(hero->direction, hero->x, hero->y + 128));
