...
void Xml::readworlds(int worldindex)
{
    ...
    for (int i = 0; i < tindex; i++)
    {
        ...
        txt << "pos" << i;
        exml->QueryIntAttribute(txt.str(), &worldteleport[i].x);
        txt.str("");
        txt << "worlddest" << i;
        exml->QueryIntAttribute(txt.str(), &worldteleport[i].worlddest);
        txt.str("");
        txt << "worldxdest" << i;
        exml->QueryIntAttribute(txt.str(), &worldteleport[i].worldxdest);
        txt.str("");
    }
