...
{
    public:
    Ether(Gfx* ingfx, Snd* insnd, Xml* inxml, Game* ingame, string intype);
    ~Ether();
...
Ether::Ether(Gfx* ingfx, Snd* insnd, Xml* inxml, Game* ingame, string intype)
{
    gfx = ingfx;
    snd = insnd;
    xml = inxml;
    ...
    if (type == "hl")
    {
        ...
        etherx = 0;
        hero = new Hero(gfx, snd, xml, "ether_hl");
    }
    else if (type == "hr")
    {
        ethersprites.push_back(sf::Sprite(gfx->etherimages[gfx->ether_h]));
        ...
        ethersprites[1].SetX(-800);
        etherx = 0;
        hero = new Hero(gfx, snd, xml, "ether_hr");
    }
    else if (type == "v")
    {
        ...
        ethery = 0;
        hero = new Hero(gfx, snd, xml, "ether_v");

        for (int i = 0; i < 77; i++)
        {
            scores.push_back(new Score(gfx, xml, "money", -sf::Randomizer::Random(xml->infolevel * 1, xml->infolevel * 4), sf::Randomizer::Random(0, 800), sf::Randomizer::Random(0, 472)));
        }
    }
    ...
    if (type == "v")
    {
        ...
        for (int i = 0; i < (int)scores.size(); i++)
        {
            if (scores[i]->y <= -64)
            {
                ...
                scores.push_back(new Score(gfx, xml, "money", -sf::Randomizer::Random(xml->infolevel * 1, xml->infolevel * 4), sf::Randomizer::Random(0, 800), 472 + 128));
            }
        }
