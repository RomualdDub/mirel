...
void Xml::readinfos()
{
    ...
    if (!xml.LoadFile())
    {
        ...
        infomoney = 0;
        infoherox = 200;
        infoherodirection = DIRECTION_RIGHT;
        infopettankx = 200 - 128;
        infopetlootx = 200 - 256;
    ...
    else
    {
        ...
        exml = exml->NextSiblingElement();
        exml->QueryIntAttribute("herodirection", &infoherodirection);
        exml = exml->NextSiblingElement();
        exml->QueryIntAttribute("pettankx", &infopettankx);
        exml = exml->NextSiblingElement();
        exml->QueryIntAttribute("petlootx", &infopetlootx);
        exml = exml->NextSiblingElement();
...
void Xml::writeinfos()
{
    ...
    if (!xml.LoadFile())
    {
        ...
        TiXmlElement* eherodir = new TiXmlElement("info");
        exml->LinkEndChild(eherodir);
        eherodir->SetAttribute("herodirection", infoherodirection);

        TiXmlElement* epettankx = new TiXmlElement("info");
        exml->LinkEndChild(epettankx);
        epettankx->SetAttribute("pettankx", infopettankx);

        TiXmlElement* epetlootx = new TiXmlElement("info");
        exml->LinkEndChild(epetlootx);
        epetlootx->SetAttribute("petlootx", infopetlootx);
    ...
    else
    {
        ...
        exml = exml->NextSiblingElement();
        exml->SetAttribute("herodirection", infoherodirection);
        exml = exml->NextSiblingElement();
        exml->SetAttribute("pettankx", infopettankx);
        exml = exml->NextSiblingElement();
        exml->SetAttribute("petlootx", infopetlootx);
        exml = exml->NextSiblingElement();
...
void Xml::readworlds(int worldindex)
{
    ...
    for (int i = 0; i < tindex; i++)
    {
        ...
        txt.str("");
        txt << "direction" << i;
        exml->QueryIntAttribute(txt.str(), &worldteleports[i].direction);
        txt.str("");
        txt << "worlddest" << i;
