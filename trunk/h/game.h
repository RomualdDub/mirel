        ...
        else
        {
            if (inv->items[sys->itemindex]->type == "gift")
            {
                ...
                if (inv->items[sys->itemindex]->level == 1) // level 1 surprise
                {
                    ...
                    inv->items[(int)inv->items.size() - 1]->name = tempitem->name;
                    inv->items[(int)inv->items.size() - 1]->setstats(sf::Randomizer::Random(0, 5), tempitem->bonus);
                    inv->items[(int)inv->items.size() - 1]->setprice(tempitem->level);
                }
                else if (inv->items[sys->itemindex]->level == 5) // level 5
                {
                    ...
                    inv->items[(int)inv->items.size() - 1]->name = tempitem->name;
                    inv->items[(int)inv->items.size() - 1]->setstats(sf::Randomizer::Random(5, 10), tempitem->bonus);
                    inv->items[(int)inv->items.size() - 1]->setprice(tempitem->level);
                }
            ...
            else if (inv->items[sys->itemindex]->type == "hat")
            {
                ...
                inv->items[(int)inv->items.size() - 1]->name = xml->currentitems[0]->name;
                inv->items[(int)inv->items.size() - 1]->setstats(xml->infodressincrementation, xml->currentitems[0]->bonus);
                inv->items[(int)inv->items.size() - 1]->setprice(xml->currentitems[0]->level);
            ...
            else if (inv->items[sys->itemindex]->type == "dress")
            {
                ...
                inv->items[(int)inv->items.size() - 1]->name = xml->currentitems[1]->name;
                inv->items[(int)inv->items.size() - 1]->setstats(xml->infodressincrementation, xml->currentitems[1]->bonus);
                inv->items[(int)inv->items.size() - 1]->setprice(xml->currentitems[1]->level);
            ...
            else if (inv->items[sys->itemindex]->type == "wand")
            {
                ...
                inv->items[(int)inv->items.size() - 1]->name = xml->currentitems[2]->name;
                inv->items[(int)inv->items.size() - 1]->setstats(xml->infodressincrementation, xml->currentitems[2]->bonus);
                inv->items[(int)inv->items.size() - 1]->setprice(xml->currentitems[2]->level);
...
void Game::addgift(Loot* loot)
{
    ...
    inv->items[(int)inv->items.size() - 1]->setstats(0, 0);
    inv->items[(int)inv->items.size() - 1]->setprice(loot->level);
}
