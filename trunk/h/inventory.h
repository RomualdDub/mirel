...
void Inventory::draw(sf::RenderWindow* app)
{
    ...
    maxpages = (int)((float)items.size() / 6) - 1;
...
void Inventory::makeinfodialogs()
{
    ...
    for (int i = 0; i < (int)items.size(); i++)
    {
        ...
        {
            messages.clear();
            txt << "Level > " << items[i]->level;
            messages.push_back(txt.str());
            txt.str("");
            txt << "Price > " << items[i]->price << "$";
            messages.push_back(txt.str());
            txt.str("");
            messages.push_back("Tips > Open me !");
            infodialogs.push_back(new Dialog(gfx, "multi", "Surprise gift", messages, items[i]->x, items[i]->y));
        }
        else if (items[i]->type == "hat" || items[i]->type == "dress" || items[i]->type == "wand")
        {
            ...
            if (items[i]->type == "hat")
                txt << "Bonus (Mana) > +" << items[i]->bonus;
            else if (items[i]->type == "dress")
                txt << "Bonus (Life) > +" << items[i]->bonus;
            else if (items[i]->type == "wand" && items[i]->bonus > 0)
                txt << "Bonus (Heal) > +" << items[i]->bonus;
            else if (items[i]->type == "wand" && items[i]->bonus < 0)
                txt << "Bonus (Damage) > " << -items[i]->bonus;
            else if (items[i]->type == "wand" && items[i]->bonus == 0)
                txt << "Bonus > +" << items[i]->bonus;

            messages.push_back(txt.str());
            txt.str("");

            if (items[i]->incrementation)
            {
                txt << items[i]->name << " + " << items[i]->incrementation;
                infodialogs.push_back(new Dialog(gfx, "multi", txt.str(), messages, items[i]->x, items[i]->y));
                txt.str("");
            }
            else
                infodialogs.push_back(new Dialog(gfx, "multi", items[i]->name, messages, items[i]->x, items[i]->y));
        }
    }
}
