                ...
                if (magics[j]->type == gfx->magic_spell0)
                {
                    if (mobs[i]->collide(magics[j]))
                    {
                        ...
                        if (mobs[i]->hp <= 0) // mob dead
                        {
                            ...
                            Loot* loot = new Loot(gfx, xml, mobs[i]->level, mobs[i]->x, mobs[i]->y);
        ...
        for (int i = 0; i < (int)mobs.size(); i++)
        {
            if (pet->collide(mobs[i]) && !mobs[i]->hit && pet->hp)
            {
                ...
                if (mobs[i]->aggro)
                {
                    ...
                    if (pet->visible)
                    {
                        scores.push_back(new Score(gfx, SCORE_LIFE, life, xml->infopetx, pet->y));
                        for (int j = 0; j < 7; j++)
                ...
                if (magics[i]->type == gfx->magic_spell0)
                {
                    if (boss[0]->collide(magics[i]))
                    {
                        ...
                        if (boss[0]->hp <= 0) // boss dead
                        {
                            ...
                            Loot* loot = new Loot(gfx, xml, boss[0]->level, boss[0]->x, boss[0]->y + 64);
                            loots.push_back(loot);
                            map->addloot(loot);
        ...
        for (int i = 0; i < (int)mobs.size(); i++)
        {
            if (mobs[i]->visible)
            {
                if (mobs[i]->collide() && mobs[i]->aggro)
                {
                    hero->hit = true;
                    int life = -game->getdamage(mobs[i]);
                    scores.push_back(new Score(gfx, SCORE_LIFE, life, xml->infoherox, hero->y));
                    game->setlife(life);
...
void Scene::keyup()
{
    for (int i = 0; i < (int)xml->worldteleports.size(); i++)
    {
        if (xml->infoherox >= xml->worldteleports[i].x && xml->infoherox <= xml->worldteleports[i].x + 64)
        {
            ...
            xml->infoworld = xml->worldteleports[i].worlddest;
...
void Scene::keyaction()
{
    ...
    {
        int manacost = 0;
        if (xml->infocurrentmagic == gfx->magic_spell0)
            manacost = 5;
        else if (xml->infocurrentmagic == gfx->magic_spell1)
            manacost = 3;
        ...
        if (game->mana >= xml->infolevel * manacost)
        {
            ...
            if (game->mana > 0)
            {
                magics.push_back(new Magic(gfx, hero->direction, xml->infoherox, hero->y + 128, xml->infocurrentmagic));
