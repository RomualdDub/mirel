...
Scene::Scene(Gfx* ingfx, Snd* insnd, Bobo* inbobo, Xml* inxml, Game* ingame, Map* inmap, System* insys)
{
    ...
    pets.push_back(new Pet(gfx, snd, xml, "hedgehog"));
    pets.push_back(new Pet(gfx, snd, xml, "crow"));
    ...
    {
        mobs.push_back(new Mob(gfx, snd, xml, hero));
        map->addmob(mobs[i]);
    ...
    boss.push_back(new Boss(gfx, snd, xml, hero));
    map->addboss(boss[0]);
            ...
            for (int j = 0; j < (int)magics.size(); j++)
            {
                if (magics[j]->type == gfx->magic_spell0)
                {
                    if (mobs[i]->collide(magics[j]))
                    {
                        ...
                        {
                            snd->explode.SetPosition(mobs[i]->x, mobs[i]->y, 0.f);
                            snd->explode.Play();
            ...
            for (int i = 0; i < (int)magics.size(); i++)
            {
                if (magics[i]->type == gfx->magic_spell0)
                {
                    if (boss[0]->collide(magics[i]))
                    {
                        ...
                        {
                            snd->explode.SetPosition(boss[0]->x, boss[0]->y, 0.f);
                            snd->explode.Play();
    ...
    for (int i = 0; i < (int)loots.size(); i++)
        if (loots[i]->visible)
        {
            if (loots[i]->collide(hero) || pets.back()->collide(loots[i]))
            {
                ...
                {
                    snd->coin.SetPosition(loots[i]->x, loots[i]->y, 0.f);
                    snd->coin.Play();
                ...
                {
                    snd->potionm.SetPosition(loots[i]->x, loots[i]->y, 0.f);
                    snd->potionm.Play();
                ...
                {
                    snd->potionl.SetPosition(loots[i]->x, loots[i]->y, 0.f);
                    snd->potionl.Play();
...
void Scene::generateloot(int inlevel, int inx, int iny)
{
    ...
    {
        Loot* loot = new Loot(gfx, snd, xml, "coin", inlevel, inx, iny);
        loots.push_back(loot);
        map->addloot(loot);
    }
    else if (luck >= 11 && luck <= 14) // potionl generated
    {
        Loot* loot = new Loot(gfx, snd, xml, "potionl", inlevel, inx, iny);
        loots.push_back(loot);
        map->addloot(loot);
    }
    else if (luck >= 15 && luck <= 18) // potionm generated
    {
        Loot* loot = new Loot(gfx, snd, xml, "potionm", inlevel, inx, iny);
        loots.push_back(loot);
        map->addloot(loot);
    }
    else if (luck >= 19 && luck <= 20) // gift generated
    {
        Loot* loot = new Loot(gfx, snd, xml, "gift", inlevel, inx, iny);
        loots.push_back(loot);
        map->addloot(loot);
    }
}
