...
int main()
{
    ...
    Scene* scene = new Scene(gfx, bobo, xml, game, map, sys);
    Ether* ether;
    ...
    while (app.IsOpened())
    {
        ...
        if (sys->ether_v)
        {
            ...
            delete scene;
            delete map;
            ether = new Ether(gfx, xml, game, "v");
        }
        else if (sys->ether_hl)
        {
            ...
            delete scene;
            delete map;
            ether = new Ether(gfx, xml, game, "hl");
        }
        else if (sys->ether_hr)
        {
            ...
            delete scene;
            delete map;
            ether = new Ether(gfx, xml, game, "hr");
        }
        if (sys->sceneether)
        {
            if (ether->type == "v")
            {
                if (ether->time >= 333)
                {
                    ...
                    delete ether;
                    map = new Map(gfx, xml);
                    scene = new Scene(gfx, bobo, xml, game, map, sys);
                }
            }
            else
            {
