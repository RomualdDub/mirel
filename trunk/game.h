...
{
    public:
    Game(int inlevel, int inexp, int inmoney, int inmagic, Bobo* inbobo);
    ~Game();
    void animate();
...
Game::Game(int inlevel, int inexp, int inmoney, int inmagic, Bobo* inbobo)
{
    ...
    coinimages.push_back(sf::Image(getimage(GFX_COIN0)));
    coinimages.push_back(sf::Image(getimage(GFX_COIN1)));
    coinimages.push_back(sf::Image(getimage(GFX_COIN2)));
    coinimages.push_back(sf::Image(getimage(GFX_COIN3)));

    for (int i = 0; i < (int)coinimages.size(); i++)
    {
        coinsprites.push_back(sf::Sprite(coinimages[i]));
    ...
    spellimages.push_back(sf::Image(getimage(GFX_INTERFACESPELL0)));
    spellimages.push_back(sf::Image(getimage(GFX_INTERFACESPELL1)));

    for (int i = 0; i < (int)spellimages.size(); i++)
    {
        spellsprites.push_back(sf::Sprite(spellimages[i]));
        spellsprites[i].SetX(26 + (i * 76));
        spellsprites[i].SetY(472 + 59);
    }
    spellindex = inmagic;

    spellsprite.SetImage(spellimages[spellindex]);
    spellsprite.SetX(52);
    spellsprite.SetY(58);
    ...
    dialogs.push_back(new Dialog("SaPik", stats, spellsprites[0].GetPosition().x, spellsprites[0].GetPosition().y));
    ...
    dialogs.push_back(new Dialog("SaHil", stats, spellsprites[1].GetPosition().x, spellsprites[1].GetPosition().y));
...
int Game::getindamage(Boss* boss)
{
    if (!sf::Randomizer::Random(0, 16))
        return 0;
    else
