...
{
    herolabel = new Label(gfx, xml);
}

void Map::addpet(Pet* pet)
{
    petlabel = new Label(gfx, xml, pet);
}
...
void Map::updatehero()
{
    herolabel->x = xml->infoherox;
}

void Map::updatepet(Pet* pet)
{
    petlabel->x = xml->infopettankx;
    petlabel->hp = pet->hp;
