...
Hero::Hero(Gfx* ingfx, Snd* insnd, Xml* inxml, string intype)
{
    ...
    hitsound.SetBuffer(snd->bufferherohit);
    ...
    repopsound.SetPitch(4.f);
	repopsound.SetMinDistance(sqrt(200.f * 200.f + 300.f * 300.f));
	repopsound.SetAttenuation(17.5f);

    footstepsound.SetBuffer(snd->bufferfootstep);
	footstepsound.SetMinDistance(sqrt(200.f * 200.f + 300.f * 300.f));
	footstepsound.SetAttenuation(17.5f);

    repopsound.SetPosition(sf::Listener::GetPosition());
    repopsound.Play();
...
void Hero::animate()
{
    sf::Listener::SetPosition(xml->infoherox, y, 300.f);
    sf::Listener::SetTarget(xml->infoherox, y, -300.f);
    ...
    else
    {
        ...
        if (hit)
        {
            if (!time)
            {
                hitsound.SetPosition(xml->infoherox, y, 0.f);
                hitsound.Play();
            }
            index = gfx->hero_hit;
            time++;
...
void Hero::dowalk()
{
    ...
    {
        footstepsound.SetPosition(xml->infoherox, y, 0.f);
        footstepsound.Play();
